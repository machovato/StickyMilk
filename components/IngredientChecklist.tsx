...
export function IngredientChecklist({
  ...
  const grouped = useMemo(() => {
    ...
    const map = new Map<string, { ing: Ingredient; originalIndex: number }[]>();
    ingredients.forEach((ing, i) => {
      const groupName = ing.group || "Ingredients";
      if (!map.has(groupName)) map.set(groupName, []);
      map.get(groupName)!.push({ ing, originalIndex: i });
    });
    return Array.from(map.entries());
  }, [ingredients, hasGroups]);

  const renderItem = (ing: Ingredient, i: number) => {
    ...
    <div className="flex flex-col gap-2">
      ...
      {grouped ? (
        <div className="space-y-4 pt-1">
          {grouped.map(([groupName, items]) => (
            <div key={groupName} className="flex flex-col gap-1.5">
              <h4 className="font-mono text-[11px] font-bold uppercase tracking-wider text-[#001ec0] bg-[#f8f2ee] px-2.5 py-1 border-l-2 border-[#001ec0]">
                {groupName}
              </h4>
              <ul className="space-y-1.5">
                {items.map(({ ing, originalIndex }) =>
                  renderItem(ing, originalIndex)
                )}
              </ul>
            </div>
          ))}
        ...
        <ul className="space-y-1.5 pt-1">
          {ingredients.map((ing, i) => renderItem(ing, i))}
        </ul>
      )}
    </div>
  );
